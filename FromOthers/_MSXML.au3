...
;                  Changed some functions to return 0 instead of -1 on fail.
...
_XMLGetChildren(ByRef $objDoc, $strXPath) Selects XML child Node(s) of an element based on XPath input from root node and returns their text values. (Requires: #include <_MSXML_DomWrapper.au3>)
...
_MSXML_NodeExists($objDoc , $strXPath) Checks for the existence of the specified path. (Requires: #include <_XMLDomWrapper.au3>)
#ce;===============================================================================
;Global variables
...
Global $DOMVERSION = -1
Global $bXMLAUTOSAVE = True;flag for autosaving feature.
...
Func _MSXML_FileOpen(ByRef $objDoc, $strXMLFile, $strNameSpc = "", $ver = -1, $bValOnParse = True)
	...
	If $objDoc.parseError.errorCode <> 0 Then
		_MSXML_Error("Error opening specified file: " & $strXMLFile & @CRLF & $objDoc.parseError.reason)
		 SetError($objDoc.parseError.errorCode,0,-1)
		 $objDoc =0
		 Return
	EndIf
...
; #FUNCTION# ===================================================================
; Description ...: Get an XML Field(s) based on XPath input from root node.
; Parameters ....: $objDoc    - Variable of type object that holds the xml document
;                  $strXPath  - xml tree path from root node (root/child/child..)
...
;~ 	Return -1
	If not IsObj($objDoc) then
		_MSXML_Error("No object passed to function _MSXML_UpdateField")
...
; #FUNCTION# =====================================================================================================================
; Description ...: Selects XML child Node(s) of an element based on XPath input from root node and returns their text values.
; Parameters ....: $objDoc    - Variable of type object that holds the xml document
;                  $strXPath  - xml tree path from root node (root/child/child..)
; Return values .: On Success - an array of child node names and values.
;                  On Failure - -1
; Author ........: Stephen Podhajecki {gehossafats at netmdc. com}
; Remarks .......: The 2 dimensional array returned includes then basename, the text value, and the namespace used by the node.
; Related .......: 
; ================================================================================================================================
Func _XMLGetChildren(ByRef $objDoc, $strXPath)
	If not IsObj($objDoc) then
		_MSXML_Error("No object passed to function _XMLGetChildren")
...
; Related .......:
;===============================================================================
Func _AddFormat( ByRef $objDoc, $objParent = "")
...
; #FUNCTION# =====================================================================================================================
; Description ...: Set automatic savaing feature on or off
; Parameters ....: $bSave - IN/OPTIONAL - 
; Return values .: Previous state.
; Author ........: Stephen Podhajecki {gehossafats at netmdc. com}
; Remarks .......: Defaults to true.
; Related .......: 
; ================================================================================================================================
Func _MSXML_SetAutoSave($bSave = True)
...
; #FUNCTION# =====================================================================================================================
; Description ...: Save the xml document
; Parameters ....: $objDoc - IN - The XML object as returned by _MSXML_Open()
;                  $strFileName - IN/OPTIONAL - The filename to save as.
; Return values .: None.
; Author ........: Stephen Podhajecki {gehossafats at netmdc. com}
; Remarks .......: If the filename parameter is empty, the currently registered filename is used.
; Related .......: 
; ================================================================================================================================
Func _MSXML_SaveDoc(ByRef $objDoc, $strFileName="")
	If $strFileName = "" Then
		$objDoc.Save($strFile)
	Else
...
; #FUNCTION# =====================================================================================================================
; Description ...: Checks for the existance of a nodec
; Parameters ....: $objDoc - IN - The XML document to check in
;                  $strXPath - IN - The node to look for,
; Return values .: On Success - A number greater than 0
;                  On Failure - 0 and @error set to 2
; Author ........: Stephen Podhajecki {gehossafats at netmdc. com}
; Remarks .......: 
; Related .......: 
; ================================================================================================================================
Func _MSXML_NodeExists(ByRef $objDoc, $strXPath)
	If not IsObj($objDoc) then
		_MSXML_Error("No object passed to function _XMLNodeExists")
