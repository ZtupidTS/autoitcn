...
#cs defs to add to au3.api
	...
	_XMLSetAttrib($strXPath, $strAttrib, $strValue = "", $iIndex =-1) Set XML Field(s) attributes based on XPath input from root node.(Requires: #include <_XMLDomWrapper.au3>)
	;==============================================================================
...
Func _XMLCreateAttrib($strXPath, $strAttrName, $strAttrValue = "")
	...
	EndIf
	Local $objNode, $err, $objAttr, $objAttrVal
	$objNode = $objDoc.selectSingleNode ($strXPath)
	If IsObj($objNode) Then
...
Func _XMLGetPath($strXPath)
	...
	If $DOMVERSION < 4 Then
		...
		Return SetError(1,0,-1)
	EndIf
	Local $objNodeList, $arrResponse[1], $xmlerr, $nodepath, $ns, $nodepathtag, $objParent
	$objNodeList = $objDoc.selectNodes ($strXPath)
	If $objNodeList.length > 0 Then
