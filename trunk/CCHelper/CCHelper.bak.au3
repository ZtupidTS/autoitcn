...
If _Singleton("chenxu_cchelper", 1) == 0 Then
	Local $stat = WinGetState("ClearCase Helper", "Merge")
	If BitAND($stat, 2) Then ; cchelper正常
		WinActivate("Rational ClearCase Explorer", "Menu bar")
		Exit
	EndIf
	Run (@ScriptDir & "\RestartCCHelper.exe", @ScriptDir)
	Exit
EndIf
...
While 1
	...
	Switch $message[0]
		...
		Case $mi_helpRestart
			Run (@ScriptDir & "\RestartCCHelper.exe", @ScriptDir)
		Case $mi_helpAbout
			MsgBox(8256,"ClearCase Helper","感谢使用。")
		Case $mi_helpHelp
			If $gui_help == "" Then
				setHelpLayout(@SW_SHOW)
			Else
				GUISetState(@SW_SHOW, $gui_help)
				GUISetState(@SW_RESTORE, $gui_help)
			EndIf
		Case $mi_helpExit
			Exit
		Case $GUI_EVENT_CLOSE
			If $message[1] == $gui_help Then GUISetState(@SW_HIDE, $gui_help)
;~ 			If $message[1] == $gui_main Then Exit
	EndSwitch
	handleMes($message[0])
WEnd
...
Func setLayout()
	$ccPid = ProcessExists("clearexplorer.exe")
	If $ccPid == 0 Then
		If Not FileExists($ccExePath) Then
			MsgBox(8208, "ClearCase Helper", "CC路径配置错误，请重新配置！")
			Run("notepad " & @ScriptDir & "\mergeConf.ini")
			Exit
		EndIf
		$ccPid = Run($ccExePath)
		WinWait("Rational ClearCase Explorer", "Menu bar")
	EndIf
	Local $ccHandle
	Do
		$ccHandle = WinGetHandle("Rational ClearCase Explorer", "Menu bar")
		Sleep(200)
	Until $ccHandle <> ""
	Local $lastPos = ControlGetPos("Rational ClearCase Explorer", "Menu bar", 59392)
	If Not IsArray($lastPos) Then ; tool bar not exist in cc explorer
		$lastPos = ControlGetPos("Rational ClearCase Explorer", "Menu bar", 59647)
		If Not IsArray($lastPos) Then
			Local $lastPos[4]
			$lastPos[0] = 0
			$lastPos[1] = 0
			$lastPos[2] = 0
	...
	$gui_main = GUICreate("ClearCase Helper", 290, $lastPos[3], $lastPos[0] + $lastPos[2] + 1, $lastPos[1], $WS_POPUP)
	DllCall("user32.dll", "int", "SetParent", "hwnd", $gui_main, "hwnd", $ccHandle)
	...
	$mi_helpRestart = GUICtrlCreateMenuItem("重启工具栏", $ctxt_help)
	GUICtrlCreateMenuItem("", $ctxt_help)
...
Func setHelpLayout($state = @SW_HIDE)
	...
	Local Const $btnHeigh = 25
	Local Const $btnWidth = 80
	...
	Local $GUIActiveX = GUICtrlCreateObj($oIE, 0, 0, $browserWidth, $browserHeigh)
...
Func merge($isMulti = False)
	...
	If WinExists("files2merge.mrgman - Merge Manager", "Ready") Then
		...
		If WinWait("Merge Manager", "将改动保存到", 3) == 0 Then
			Local $pid = WinGetProcess("files2merge.mrgman - Merge Manager")
			ProcessClose($pid)
		EndIf
...
Func getCCPath()
	...
	Local $parentNode, $nodeText, $tmpStr, $rootText = _GUICtrlTreeView_GetText($hWnd, 0)
	While 1
...
;~ 	GUICtrlSetState($btn_move, $GUI_DISABLE)
EndFunc
...
;~ 	GUICtrlSetState($btn_move, $GUI_ENABLE)
EndFunc
...
Func move($initX, $initY)
;~ Opt("MouseCoordMode", 0)
	Local $text = GUICtrlRead($btn_merge)
	Do
		Local $nMsg = GUIGetMsg()
		Local $mousePos = MouseGetPos()
		WinMove("ClearCase Helper", $text, $mousePos[0] - $initX, $mousePos[1] - $initY)
;~ 		ConsoleWrite($nMsg[3] & ", " & $nMsg[4] & @CRLF)
	Until $nMsg == $GUI_EVENT_PRIMARYUP
EndFunc
...
Func _searchPrivate()
	If Not FileExists($ccHome & "\bin\cleartool.exe") Then
		MsgBox(8240, "ClearCase Helper", $ccHome & "\bin\cleartool.exe 不存在，请确认ClearCase安装正确。")
		Return
	EndIf
	
	Local $path = WinGetTitle("Rational ClearCase Explorer", "Menu bar")
	$path = StringMid($path, StringInStr($path, "(") + 1)
	$path = StringMid($path, 1, StringLen($path) - 1)
	If Not FileExists($path) Then
		MsgBox(8240, "ClearCase Helper", "无法获取当前选择的路径，未知错误。")
		Return
	EndIf
	
	Local $gui_ctls = GUICreate("扫描结果 - 0", 600, 480)
	GUICtrlCreateLabel("扫描目录", 1, 5)
	GUICtrlCreateInput($path, 60, 1, 539, Default, $ES_READONLY)
	GUICtrlCreateLabel("被Check Out的文件：", 1, 25)
	GUICtrlCreateLabel("私有文件：", 1, 258)
	Local $txt_co = GUICtrlCreateEdit("正在扫描，请稍等...", 1, 40, 598, 209, _
		BitOR($ES_AUTOVSCROLL,$ES_AUTOHSCROLL,$ES_READONLY,$ES_WANTRETURN,$WS_HSCROLL,$WS_VSCROLL))
	Local $txt_private = GUICtrlCreateEdit("正在扫描，请稍等...", 1, 272, 598, 209, _
		BitOR($ES_AUTOVSCROLL,$ES_AUTOHSCROLL,$ES_READONLY,$ES_WANTRETURN,$WS_HSCROLL,$WS_VSCROLL))
	GUISetState(@SW_SHOW, $gui_ctls)
	
	Local $ctls = Run('"' & $ccHome & '\bin\cleartool.exe" ls -r -view_only ' & $path, $ccHome & "\bin", @SW_HIDE, $STDOUT_CHILD)
	Local $line, $privateFiles = "", $n = 0
	While 1
		$line = StdoutRead($ctls)
		If @error Then ExitLoop
		$privateFiles &= $line
		StringReplace($line, @CRLF, @CRLF)
		If @extended == 0 Then ContinueLoop
		$n += @extended
		WinSetTitle ("扫描结果 - ", "正在扫描，请稍等...", "扫描结果 - " & $n)
	Wend
	WinSetTitle ("扫描结果 - ", "正在扫描，请稍等...", "扫描结果 - " & $n & " 完成")
	Local $aArray = StringSplit($privateFiles, @CRLF, 1) ; Try Windows @CRLF first
	...
	For $i = 1 To $aArray[0]
		If StringInStr($aArray[$i], "Rule: CHECKEDOUT") Then
			...
			$aArray[$i] = StringLeft($aArray[$i], $n)
			$co &= StringReplace($aArray[$i], $path, ".") & @CRLF
		Else
			$private &= StringReplace($aArray[$i], $path, ".") & @CRLF
		EndIf
	Next
	GUICtrlSetData($txt_co, $co)
	GUICtrlSetData($txt_private, $private)
...
Func launchCQ()
	If WinExists("Rational ClearQuest", "Standard") Then
		WinActivate("Rational ClearQuest", "Standard")
		Return
	EndIf
