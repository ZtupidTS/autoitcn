...
function KindDrawMedia(cmd)
{
	...
	ks.append('<tr><td style="width:40px;padding:5px;">', KE_LANG['ADDRESS'], '</td>');
...
function KindPopupMenu(cmd)
{
	switch (cmd)
	{
		...
		case 'KE_ABOUT':
			...
			str += '<span style="margin-right:10px;">KindEditor ' + KE_VERSION + '，感谢kindsoft出色的工作，编写出优秀的HTML在线编辑工具。</span>' + 
...
function KindDrawIframe(cmd)
{
	switch (cmd)
	{
		case 'KE_IMAGE':
			...
			str += KE_LANG['ADDRESS'] + '</td><td style="width:200px;padding-bottom:3px;">';
		...
		case 'KE_FLASH':
			...
			'<tr><td style="width:40px;padding:5px;">'+KE_LANG['ADDRESS']+'</td>' +
		...
		case 'KE_LINK':
			...
			str += '<table cellpadding="0" cellspacing="0" style="width:100%;font-size:12px;">' + 
				...
				'<td style="width:200px;padding-top:5px;padding-bottom:5px;"><input type="text" id="hyperLink" value="../" style="width:190px;border:1px solid #555555;background-color:#FFFFFF;"></td>' +
				'<tr><td style="padding:5px;">'+KE_LANG['TARGET']+'</td>' +
				'<td style="padding-bottom:5px;"><select id="hyperLinkTarget"><option value="" selected="selected">'+KE_LANG['CURRENT_WINDOW']+'</option><option value="_blank">'+KE_LANG['NEW_WINDOW']+'</option></select></td></tr>' + 
...
{
	var url = KE_IMAGE_DOCUMENT.getElementById('imgFile').value;
...
function KindEditor(objName) 
{
	...
	this.data = function()
	{
		...
		document.getElementsByName("source_code")[0].value = htmlResult;
		return htmlResult;
	}
}
